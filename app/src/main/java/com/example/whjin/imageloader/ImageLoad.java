package com.example.whjin.imageloader;
...
public class ImageLoad {
    ...
    /**
     * build a new instance of ImageLoad
     * @Param context
     * @return a new instance of ImageLoad
     */
     ...
     * NOTE THAT: should run in Thread
     * @param uri http uri
    ...
    public Bitmap loadBitmap(String uri, int reqWidth, int reqHeight) {
        ...
        if (null==bitmap && !mIsDiskLruCacheCreated) {
    ...
    private Bitmap loadBitmapFromDiskCache(String uri, int reqWidth, int reqHeight) throws IOException {
        if (Looper.myLooper() == Looper.getMainLooper()) {
            Log.w("TAG", "load bitmap from UI Thread, it's recommended");
        }
        if (mDiskLruCache == null) {
            return null;
        }
        ...
        Bitmap bitmap = null;
        String key = hashKeyFromUri(uri);
        DiskLurCache.Snapshot snapshot = mDiskLruCache.get(key);
        if (snapshot != null) {
